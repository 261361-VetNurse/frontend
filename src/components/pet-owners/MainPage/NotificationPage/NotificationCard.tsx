  ...
  return (
    ...
    >
      <div className="w-11 h-11 rounded-full overflow-hidden flex-shrink-0 bg-gray-100 mr-4 border border-gray-100">
        <img
          src={item.image_url || "/images/lee.png"}
          alt={item.title}
          className="w-full h-full object-cover"
          onError={(e) => {
            // Fallback if image fails
            (e.target as HTMLImageElement).src = "/images/lee.png";
          }}
        />
      </div>
      <div className="flex-grow">
        <div className="flex justify-between items-start">
          ...
          <span className="text-[10px] text-gray-400 font-normal">{dayjs(item.created_at).fromNow()}</span>
        </div>
        <p className="text-gray-500 text-[12px] mt-[2px] leading-tight">{item.message}</p>
      </div>
