...
Cypress.Commands.add('fiGetAuthToken', () => {
  ...
  return cy
    .task('fi:token', undefined, { timeout: 60000 })
    .then((token) => {
  ...
  return cy
    ...
    .then(() =>
      cy.fiApi('PATCH', '/v1/user/profile', {
        ...
        email: payload.email,
        gender: 'female',
        address: `${payload.address_line1} ${payload.subdistrict}`,
      }, { failOnStatusCode: false })
    )
    .then((res) => {
...
Cypress.Commands.add(
  ...
  const developerAccessFallback = () => {
    cy.log('fi:token failed -> fallback Developer Access (DEV_1)');
  ...
  return cy
    ...
    .then(() => cy.task('fi:tokenSafe', undefined, { timeout: 60000 }))
    .then((result: any) => {
      if (result?.ok && result.token) {
        const token = String(result.token);
        Cypress.env('fiAuthTokenCache', token);
        return visitWithToken(token);
      }
